/* ---------------------------------------------------------------------
 * $Id: step-32.cc 30306 2013-08-13 19:55:44Z heister $
 *
...
namespace Step32
{
  ...
  namespace Assembly
  {
    namespace Scratch
    {
      ...
      struct DepthAverage
      {
        DepthAverage (const FiniteElement<dim> &temperature_fe,
		      const Quadrature<dim>    &temperature_quadrature,
		      const Mapping<dim>       &mapping,
		      const UpdateFlags         update_flags);

        DepthAverage (const DepthAverage &data);
      ...
      DepthAverage (const FiniteElement<dim> &temperature_fe,
		    ...
		    const UpdateFlags         update_flags)
        :
        temperature_fe_values (mapping, temperature_fe, temperature_quadrature,
			       update_flags),
        temperature_values (temperature_quadrature.size())
      {}
      ...
      template <int dim>
      DepthAverage<dim>::
      DepthAverage (const DepthAverage &scratch)
        :
        temperature_fe_values (scratch.temperature_fe_values.get_mapping(),
                          scratch.temperature_fe_values.get_fe(),
                          scratch.temperature_fe_values.get_quadrature(),
                          scratch.temperature_fe_values.get_update_flags()),
        temperature_values (scratch.temperature_values.size())
      {}
	...
	StokesSystem() = default;
      ...
      template <int dim>
      struct StokesSystemOther : public StokesSystem<dim>
      {
        StokesSystemOther (const FiniteElement<dim> &stokes_fe,
                      const Mapping<dim>       &mapping,
                      const Quadrature<dim>    &stokes_quadrature,
                      const UpdateFlags         stokes_update_flags,
                      const FiniteElement<dim> &temperature_fe,
                      const UpdateFlags         temperature_update_flags)
          :
          StokesSystem<dim> (stokes_fe, mapping, stokes_quadrature,
                             stokes_update_flags, temperature_fe,
                             temperature_update_flags),
        ...
        StokesSystemOther (const StokesSystemOther<dim> &data)
          :
          StokesSystem<dim> (data),
          local_matrix(data.local_matrix),
          local_dof_indices(data.local_dof_indices),
          local_rhs(data.local_rhs)
        {}

	      StokesSystemOther() = default;
  ...
  private:
    ...
    std::vector<std::vector<FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> > >
      temperature_graph;
    ...
    void
    local_assemble_stokes_system_other (const MatrixFree<dim> &,
                                        Threads::ThreadLocalStorage<Assembly::Scratch::StokesSystemOther<dim> >  &scratch,
                                        const int &,
    ...
    void
    local_compute_depth_average (const typename DoFHandler<dim>::active_cell_iterator &cell,
				 Assembly::Scratch::DepthAverage<dim> &scratch,
				 Assembly::CopyData::DepthAverage &data);

    static
    void
    copy_local_to_global_depth_average (const Assembly::CopyData::DepthAverage &data,
					std::vector<double> &depth_averages,
					std::vector<double> &volumes);


    std::vector<types::global_dof_index>
    get_conflict_indices (FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> const &cell);

    static
    unsigned int
    depth_average_conflict_indices (FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> const &cell);

    void build_temperature_graph();

    void build_stokes_graph();
  ...
  parse_parameters (ParameterHandler &prm)
  {
    ...
    if (dim == 3 && initial_global_refinement > 4)
      initial_global_refinement -= 4;
  ...
  void BoussinesqFlowProblem<dim>::project_temperature_field ()
  {
    ...
    TrilinosWrappers::MPI::Vector rhs (temperature_rhs),
        solution (temperature_rhs);
  ...
  BoussinesqFlowProblem<dim>::assemble_stokes_preconditioner ()
  {
    ...
    computing_timer.enter_subsection ("x-assemble-stokes-precon");
  ...
  void
  BoussinesqFlowProblem<dim>::
  local_assemble_stokes_system_other (const MatrixFree<dim> &/*matrix_free*/,
                                Threads::ThreadLocalStorage<Assembly::Scratch::StokesSystemOther<dim> > &/*in_scratch*/,
                                const int &,
                                const std::pair<unsigned int,unsigned int>&/*cell_range*/)
  {
/*    Assembly::Scratch::StokesSystemOther<dim> &scratch = in_scratch.get();
    const unsigned int dofs_per_cell = scratch.stokes_fe_values.get_fe().dofs_per_cell;
    const unsigned int n_q_points    = scratch.stokes_fe_values.n_quadrature_points;
  ...
  for (unsigned int mcell=cell_range.first; mcell<cell_range.second; ++mcell)
    for (unsigned int vec=0; vec<1;++vec)//matrix_free.n_components_filled(mcell); ++vec)
      {
        typename DoFHandler<dim>::active_cell_iterator
          temperature_cell = matrix_free.get_cell_iterator (mcell, vec, 0);
        typename DoFHandler<dim>::active_cell_iterator
          cell (&triangulation,
                temperature_cell->level(),
                temperature_cell->index(),
                &stokes_dof_handler);
    ...
    if (rebuild_stokes_matrix)
      scratch.local_matrix = 0;
    for (unsigned int i=0; i<dofs_per_cell; ++i)
      scratch.local_rhs(i) = 0;
    ...
    for (unsigned int q=0; q<n_q_points; ++q)
      {
        ...
        if (rebuild_stokes_matrix == true)
          for (unsigned int i=0; i<dofs_per_cell; ++i)
            for (unsigned int j=0; j<dofs_per_cell; ++j)
              scratch.local_matrix(i,j) += (EquationData::eta * 2 *
                                         (scratch.grads_phi_u[i] * scratch.grads_phi_u[j])
                                         - (EquationData::pressure_scaling *
                                            scratch.div_phi_u[i] * scratch.phi_p[j])
                                         - (EquationData::pressure_scaling *
        ...
        for (unsigned int i=0; i<dofs_per_cell; ++i)
          scratch.local_rhs(i) += (EquationData::density(old_temperature) *
                                gravity  *
                                scratch.phi_u[i]) *
                               scratch.stokes_fe_values.JxW(q);
      }
    ...
    if (rebuild_stokes_matrix == true)
      stokes_constraints.distribute_local_to_global (scratch.local_matrix,
                                                     scratch.local_rhs,
                                                     scratch.local_dof_indices,
                                                     stokes_matrix,
                                                     stokes_rhs);
    else
      stokes_constraints.distribute_local_to_global (scratch.local_rhs,
                                                     scratch.local_dof_indices,
                                                     stokes_rhs);
      }*/
  }
  ...
  {
    computing_timer.enter_subsection ("   Assemble Stokes system");
    ...
    computing_timer.enter_subsection ("x-assemble-stokes-system");
#ifdef implementation2
    WorkStream::
    run (CellFilter (IteratorFilters::LocallyOwnedCell(),
                     stokes_dof_handler.begin_active()),
    ...
    additional_data.tasks_block_size = 4;
    matrix_free.reinit(mapping, temperature_dof_handler, temperature_constraints,
                       quad, additional_data);
    if (rebuild_stokes_matrix == true)
      stokes_matrix=0;
    stokes_rhs=0;
    computing_timer.enter_subsection ("mf-assemble-stokes-system");
    Assembly::Scratch::
      StokesSystemOther<dim> scratch (stokes_fe, mapping, quadrature_formula,
                                 (update_values    |
                                  update_quadrature_points  |
                                  update_JxW_values |
                                  (rebuild_stokes_matrix == true
                                 ...
                                 temperature_fe,
                                 update_values);
    Threads::ThreadLocalStorage<Assembly::Scratch::StokesSystemOther<dim> > data(scratch);
    int dummy = 0;
    matrix_free.cell_loop(&BoussinesqFlowProblem<dim>::local_assemble_stokes_system_other,
                          this, data, dummy);
    computing_timer.leave_subsection();
  ...
  void BoussinesqFlowProblem<dim>::assemble_temperature_matrix ()
  {
    ...
    computing_timer.enter_subsection ("   Assemble temperature matrices");
    ...
    computing_timer.enter_subsection ("x-assemble-temperature-matrix");
#ifdef implementation2
    WorkStream::
    run (CellFilter (IteratorFilters::LocallyOwnedCell(),
  ...
  depth_average_conflict_indices (FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> const &cell)
  {
    unsigned int interval = std::max(0,
				     static_cast<int>((Point<dim>().distance(cell->center()) - EquationData::R0) /
						      (EquationData::R1- EquationData::R0) * 100));
    if (interval > 100)
      interval = 100;
  ...
  local_compute_depth_average (const typename DoFHandler<dim>::active_cell_iterator &cell,
			       ...
			       Assembly::CopyData::DepthAverage &data)
  {
    Assert (scratch.temperature_fe_values.n_quadrature_points == 1, ExcInternalError());

    scratch.temperature_fe_values.reinit (cell);
    scratch.temperature_fe_values.get_function_values (temperature_solution,
						       scratch.temperature_values);
    data.value = scratch.temperature_values[0] * scratch.temperature_fe_values.JxW(0);
    data.volume = cell->measure();
    data.interval = std::max(0,
			     static_cast<int>((Point<dim>().distance(cell->center()) - EquationData::R0) /
					      (EquationData::R1- EquationData::R0) * 100));
  ...
  copy_local_to_global_depth_average (const Assembly::CopyData::DepthAverage &data,
					...
					std::vector<double> &volumes)
  {
    depth_averages[data.interval] += data.value;
    volumes[data.interval] += data.volume;
  }


  template <int dim>
  void BoussinesqFlowProblem<dim>::build_temperature_graph ()
  {
    Timer t;
    t.start();
    typedef FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> CellFilter;
    CellFilter begin(IteratorFilters::LocallyOwnedCell(),temperature_dof_handler.begin_active());
    CellFilter end(IteratorFilters::LocallyOwnedCell(),temperature_dof_handler.end());
    temperature_graph = GraphColoring::make_graph_coloring(begin,end,
							   [this] (const CellFilter &cell) -> std::vector<types::global_dof_index>
							   {
							     return this->get_conflict_indices(cell);});
    
    t.stop();
    
    std::cout << "Temp graph:"
	      << temperature_graph.size() << std::endl;
    for (unsigned int i=0; i<temperature_graph.size(); ++i)
      std::cout << temperature_graph[i].size() << ' ';
    std::cout << " in " << t.wall_time() << " seconds";
    std::cout << std::endl;
  }

  template <int dim>
  void BoussinesqFlowProblem<dim>::build_stokes_graph()
  {
    Timer t;
    t.start();
    typedef FilteredIterator<typename DoFHandler<dim>::active_cell_iterator> CellFilter;
    CellFilter begin(IteratorFilters::LocallyOwnedCell(),stokes_dof_handler.begin_active());
    CellFilter end(IteratorFilters::LocallyOwnedCell(),stokes_dof_handler.end());
    stokes_graph = GraphColoring::make_graph_coloring(begin,end,
							   [this] (const CellFilter &cell) -> std::vector<types::global_dof_index>
							   {
							     return this->get_conflict_indices(cell);});
    ...
    std::cout << "Stokes graph:"
	      << stokes_graph.size() << std::endl;
    for (unsigned int i=0; i<stokes_graph.size(); ++i)
      std::cout << stokes_graph[i].size() << ' ';
    std::cout << " in " << t.wall_time() << " seconds";
    std::cout << std::endl;
  }
  ...
  void BoussinesqFlowProblem<dim>::assemble_temperature_system (const double maximal_velocity)
  {
    ...
    computing_timer.enter_subsection ("x-assemble-temperature-rhs");
auto worker =
  ...
  void BoussinesqFlowProblem<dim>::solve ()
  {
    ...
    computing_timer.enter_subsection ("   Assemble temperature rhs");
    {
    ...
    if (solve_systems)
    {
      computing_timer.enter_subsection ("   Solve temperature system");
      ...
      for (unsigned int i=distributed_temperature_solution.local_range().first; i<distributed_temperature_solution.local_range().second; ++i)
        {
  ...
  template <int dim>
  void
  BoussinesqFlowProblem<dim>::Postprocessor::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
  ...
  //                                    std::vector<Vector<double> >                    &computed_quantities) const
  {
    ...
    Assert (input_data.solution_gradients.size() == n_quadrature_points,                  ExcInternalError());
    Assert (computed_quantities.size() == n_quadrature_points,  ExcInternalError());
    Assert (input_data.solution_values[0].size() == dim+2,                              ExcInternalError());

    for (unsigned int q=0; q<n_quadrature_points; ++q)
      {
        for (unsigned int d=0; d<dim; ++d)
          computed_quantities[q](d)
            = (input_data.solution_values[q](d) *  EquationData::year_in_seconds * 100);

        const double pressure = (input_data.solution_values[q](dim)-minimal_pressure);
        computed_quantities[q](dim) = pressure;

        const double temperature = input_data.solution_values[q](dim+1);
  ...
  {
    pcout << "   Generating graphical output..." << std::endl;
    ...
    Postprocessor postprocessor (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD),
                                 0.0);//stokes_solution.block(1).minimal_value());

    DataOut<dim> data_out;
    data_out.attach_dof_handler (joint_dof_handler);
    data_out.add_data_vector (locally_relevant_joint_solution, postprocessor);
    computing_timer.enter_subsection ("x-data-out");
    ...
    // now compute a depth average of the temperature. to this end,
    // split the interval R0...R1 into 100 subsets
    pcout << "   Doing depth average -- graph..." << std::endl;

    std::vector<double> depth_averages(101);
    std::vector<double> volumes(101);

    QMidpoint<dim> quadrature_formula;
    typedef
    FilteredIterator<typename DoFHandler<dim>::active_cell_iterator>
    CellFilter;
...
/*
    {
      CellFilter cell(IteratorFilters::LocallyOwnedCell(),temperature_dof_handler.begin_active());
      CellFilter end(IteratorFilters::LocallyOwnedCell(),temperature_dof_handler.end());

      std::vector<std::vector<CellFilter> >
	partitions (100);
      for (; cell!=end; ++cell)
	partitions[depth_average_conflict_indices(cell)].push_back (cell);

      unsigned int n_colors = 0;
      for (unsigned int i=0; i<partitions.size(); ++i)
	n_colors = std::max<unsigned int> (n_colors, partitions[i].size());
      depth_graph.resize (n_colors);

      for (unsigned int c=0; c<n_colors; ++c)
	for (unsigned int i=0; i<partitions.size(); ++i)
	  if (c < partitions[i].size())
	    depth_graph[c].push_back (partitions[i][c]);
    }*/
#endif
    pcout << "   Doing depth average -- compute..." << std::endl;

    computing_timer.enter_subsection ("x-depth-average");
    /*#ifdef implementation2
    WorkStream::run (CellFilter (IteratorFilters::LocallyOwnedCell(),
		     ...
		     std::bind (&BoussinesqFlowProblem<dim>::
				      local_compute_depth_average,
				      this,
				      std::_1,
				      std::_2,
				      std::_3),
		     std::bind (&BoussinesqFlowProblem<dim>::
				      copy_local_to_global_depth_average,
				      std::_1,
				      std::ref(depth_averages),
				      std::ref(volumes)),
		     Assembly::Scratch::
		     DepthAverage<dim> (temperature_fe, quadrature_formula,
					mapping,
					update_JxW_values |
					update_values),
		     Assembly::CopyData::DepthAverage ());
#else
    WorkStream::run (depth_graph,
		     std::bind (&BoussinesqFlowProblem<dim>::
				      local_compute_depth_average,
				      this,
				      std::_1,
    ...
    const std::string d_a_filename = ("depth_average-" +
				      Utilities::int_to_string (out_index, 5));
    std::ofstream d_a (d_a_filename.c_str());
    for (unsigned int i=0; i<depth_averages.size(); ++i)
      d_a << depth_averages[i] / volumes[i] << std::endl;
...
#ifndef implementation2
    // Mesh are not changed -> we can build the two graphs once
...
#else
  std::cout<<"Implementation 3"<<std::endl;
#endif

  std::cout << "************ Running with nthreads=" << atoi(nthreads) << std::endl;
  Utilities::MPI::MPI_InitFinalize mpi_initialization(argc, argv, atoi(nthreads));
